...
h_BB_per80_20_dvdt=array('f',[0])
h_BB_per80_20_time=array('f',[0])
...
h_CSA_per80_20_dvdt=array('f',[0])
h_CSA_per80_20_time=array('f',[0])
...
h_CSA_per80_20_dvdt[0]=0.0
h_CSA_per80_20_time[0]=0.0

h_BB_per80_20_dvdt[0]=0.0
h_BB_per80_20_time[0]=0.0
...
class AddNoise:
    def __init__(self):
        ...
        self.per80_20_time = {"BB":0, "CSA":0}
        self.per80_20_dvdt = {"BB":0, "CSA":0}
...
class RootFile:
    ...
    def root_define(self):
        ...
        self.tree_out.Branch('h_BB_per80_20_dvdt',
                              h_BB_per80_20_dvdt,
                             'h_BB_per80_20_dvdt/F')
        self.tree_out.Branch('h_BB_per80_20_time',
                              h_BB_per80_20_time,
                             'h_BB_per80_20_time/F')
        self.tree_out.Branch('h_CSA_per80_20_dvdt',
                              h_CSA_per80_20_dvdt,
                             'h_CSA_per80_20_dvdt/F')
        self.tree_out.Branch('h_CSA_per80_20_time',
                              h_CSA_per80_20_time,
                             'h_CSA_per80_20_time/F')
    ...
    def fill_ampl_BB(self,addNoise,rset,max_height,max_time):
        ...
        h_BB_per80_20_time[0]=addNoise.per80_20_time["BB"]
        addNoise.BB_Fv=1
...
def get_CFD_time(addNoise,Ampl_paras,rset,model):
    ...
    for i in range (0,len(time_list)):
        ...
        if Ampl_paras[model_list][i]>=Ampl_paras[model_height]*0.2 \
           and time_list[i]<Ampl_paras[max_time] and CFD20 == 0:
            per20_time = time_list[i]
            per20_ampl = Ampl_paras[model_list][i]
            CFD20 = 1

        if Ampl_paras[model_list][i]>=Ampl_paras[model_height]*0.8 \
           and time_list[i]<Ampl_paras[max_time] and CFD80 == 0:
            per80_time = time_list[i]
            per80_ampl = Ampl_paras[model_list][i]
            CFD80 = 1            
    ...
    if CFD20 == 1 and CFD80 == 1:
        addNoise.per80_20_time[model] =  per80_time - per20_time 
        if per80_time - per20_time > 0:
            addNoise.per80_20_dvdt[model] =  (per80_ampl - per20_ampl) /  (per80_time - per20_time)     
        else:
            addNoise.per80_20_dvdt[model] = 0.0
    else:
        addNoise.per80_20_time[model] = 0.0
        addNoise.per80_20_dvdt[model] = 0.0
